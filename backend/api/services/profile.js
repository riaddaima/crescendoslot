...
const createUserProfile = async (profile) => {
  try {
    const { rows } = await db.query('INSERT INTO profiles (pro_phone, pro_issubbed, usr_id) VALUES ($1, $2, $3) RETURNING *', [profile.pro_phone, profile.pro_issubbed, profile.usr_id]);
    return rows[0];
  } catch (error) {
    throw error;
  }
...
const updateUserProfile = async (userId, profile) => {
  try {
    const { rows } = await db.query('UPDATE profiles SET (pro_phone, pro_issubbed) = ($1, $2) WHERE usr_id = $4 RETURNING *', [profile.pro_phone, profile.pro_issubbed, userId]);
    return rows[0];
  } catch (error) {
    throw error;
  }
